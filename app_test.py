...
class PortfolioTestCase(unittest.TestCase):
    ...
    def test_create_new_project_success(self):
        res = self.client().post('/projects', json=self.new_good_project
                                 , headers=self.headers)
        self.assertEqual(res.status_code, 200)

    # creating a new project failing
    def test_create_new_project_fail(self):
        res = self.client().post('/projects', json=self.new_bad_project,
                                 headers=self.headers)
    ...
    def test_update_project_success(self):
        ...
        res = self.client().patch('/projects/' + id, json=self.new_good_project,
                                  headers=self.headers)
        self.assertEqual(res.status_code, 200)

    # update a project failing
    def test_update_project_fail(self):
        res = self.client().patch('/projects/-1', json=self.new_bad_project,
                                  headers=self.headers)
    ...
    def test_delete_project_success(self):
        id = str(Project.query.all()[0].id)
        res = self.client().delete('/projects/' + id,
                                   headers=self.headers)
        self.assertEqual(res.status_code, 200)

    # delete a project failing
    def test_delete_project_fail(self):
        res = self.client().delete('/projects/-1',
                                   headers=self.headers)
